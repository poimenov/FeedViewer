...
module Types =
    ...
    type NextPrevLogic(store: IShareStore, jsRuntime: IJSRuntime) =
        ...
        let moveTo f chItems chItem =
            ...
            | None -> ()
            | Some item ->
                store.SelectedChannelItem.Publish(Selected item)
                jsRuntime.InvokeAsync "ScrollToSelectedItem" |> ignore

        member this.MoveToNext =
            moveTo Next store.FeedItems.Value store.SelectedChannelItem.Value

        member this.MoveToPrevious =
            moveTo Previous store.FeedItems.Value store.SelectedChannelItem.Value

        member this.IsNextDisabled =
            isDisabled Next store.FeedItems.Value store.SelectedChannelItem.Value

        member this.IsPreviousDisabled =
            isDisabled Previous store.FeedItems.Value store.SelectedChannelItem.Value